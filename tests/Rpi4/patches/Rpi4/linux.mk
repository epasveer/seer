...
# QuangNM: apply patch for debugging kernel module on Rpi4
define CONFIG_QUANGNM_ENABLE_DEBUG
	echo "CONFIG_DEBUG_INFO=y" 			>> $(@D)/.config
	echo "CONFIG_EXPERT=y" 				>> $(@D)/.config
	echo "CONFIG_KALLSYMS=y" 			>> $(@D)/.config
	echo "CONFIG_SYSFS=y" 				>> $(@D)/.config
	echo "CONFIG_PID_IN_CONTEXTIDR=y" 	>> $(@D)/.config
	echo "CONFIG_IKCONFIG_PROC=y" 		>> $(@D)/.config
	echo "CONFIG_CORESIGHT_CPU_DEBUG=y" >> $(@D)/.config
	echo "CONFIG_KALLSYMS_ALL=y" 		>> $(@D)/.config
	echo "CONFIG_DEBUG_KERNEL=y" 		>> $(@D)/.config
	echo "#Quangnm13 debug " 		>> $(@D)/.Makefile
	$(SED) 's|MAKEFLAGS += -rR|MAKEFLAGS += -g -rR|g' $(@D)/Makefile
	if ! grep -q '^KBUILD_CFLAGS := -g' $(@D)/Makefile; then \
		$(SED) 's/^KBUILD_CFLAGS :=/KBUILD_CFLAGS := -g -O0 /' $(@D)/Makefile; \
	fi	
endef
